---
layout: post
title:  "Calling C functions from Python - part 2 - writing python/C extensions"
date:   2018-01-08
description: Calling C functions from Python - part 2 - writing python/C extensions
...
Py_InitModule4TraceRefs_64 only exists in debug libs.

At this point I gave up and simply build release instead. In my opinion this is not designed properly - _DEBUG should control debug-ness of the module and python itself should use a different one.
...
* It is itself an PyObject 

Just look at the `PyVarObject_HEAD_INIT` macro that initializes PyObject's fields. 
...
As you can see above, it is giving a 1 ref count and NULL type (which we'll fill in later). All Python objects are ref-counted and as a extension writer we need to maintain ref count correctly.

* `tp_name` field defines the class name - "fastint.fastInt"


`tp_new` field defines the new function - FastInt_New:
...
The new function does just the allocation - think it as `malloc` or `new` without constructor (placement new, if you are familiar with C++). Conveniently, we'll use the type object of fastint type to allocate a type object. 
